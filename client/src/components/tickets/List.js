...
export default class TicketList extends React.Component{
    ...
    softRemove=(id)=>{
        const ticket1=this.state.tickets.map(e1=>e1._id==id?{...e1,isDeleted:true}:e1)
        console.log(ticket1)
        ...
        // }
        // console.log(data)
        axios.put(`/tickets/softdelete/${id}`,ticket_body,{
            headers:{
                'x-auth':localStorage.getItem('authToken')
            }
        })
    ...
    handleRestore=()=>{
        ...
        .then(response=>{
            const True=[]
            response.data.forEach(r1=>{if(r1.isDeleted==true)True.push(r1._id)})
            console.log(True)
        ...
        return(
            <div>
                ...
                <table>
                    ...
                    <tbody>
                        {this.state.tickets.filter(e1=>e1.isDeleted==false).map((ticket,index)=>{
                            return(
                                <tr key={ticket._id}>
                                    <td>{index+1}</td>
                                    ...
                                    <td>{ticket.employeesIds.map(emp=>{
                                        return emp.name
                                    }).join(',')}</td>
                                    <td>{ticket.message}</td>
                                    <td>{ticket.priority}</td>
                                    <td></td>
                                    <td><button onClick={()=>{this.handleRemove(ticket._id)}}>Remove</button></td>
                                    <td></td>
                                    <td><button onClick={()=>{
                                        this.softRemove(ticket._id)
                                    }}>Soft Delete</button></td>
                                </tr>
                            )
                        })}
                    </tbody>
                </table>
                <button onClick={()=>{this.handleRestore()}}>Restore</button><br/>
